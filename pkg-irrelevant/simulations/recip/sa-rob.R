...
## sa.hyper <- multiSA(objective, proposalMonoFac(indicator), rar.m.hyper,
##                     schedule, 10000, 200)
...
sa.hyper <- mcmapply(multiSA,
                     proposal = prop,
                     MoreArgs = list(objf = objective,
                                     schedule = recipSchedule,
                                     starting = rar.m.hyper,
                                     N = 1000,
                                     iter = 400),
                     SIMPLIFY = FALSE,
                     mc.cores = 15)
saveRDS(sa.hyper, "./simulations/recip/sa-hyper-rob.rds")
